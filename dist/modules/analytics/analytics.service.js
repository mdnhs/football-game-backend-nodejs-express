...
async function getDashboardStats() {
    ...
    const [totalPlayers, todayScores, flaggedScores, avgScore] = await Promise.all([
        supabase_1.supabase.from("players").select("id", { count: "exact", head: true }),
        supabase_1.supabase
            .from("scores")
        ...
        supabase_1.supabase
            ...
            if (!data?.length)
                return 0;
            return Math.round(data.reduce((s, r) => s + (r.total_score ?? 0), 0) / data.length);
        }),
    ]);
    return {
