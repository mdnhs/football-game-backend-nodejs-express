...
exports.QrRefConflictError = exports.QrNotFoundError = void 0;
exports.buildQrUrl = buildQrUrl;
exports.createQrCode = createQrCode;
exports.listQrCodes = listQrCodes;
exports.deactivateQrCode = deactivateQrCode;
exports.activateQrCode = activateQrCode;
...
exports.QrNotFoundError = QrNotFoundError;
class QrRefConflictError extends Error {
    constructor() {
        super("QR ref already exists");
        this.name = "QrRefConflictError";
    }
}
exports.QrRefConflictError = QrRefConflictError;
function buildQrUrl(ref) {
    return `${env_1.env.FRONTEND_URL.replace(/\/$/, "")}/?ref=${encodeURIComponent(ref)}`;
}
...
async function listQrCodes(page = 1, limit = 50) {
...
async function getQrStats(id) {
    const { data: qr, error } = await supabase_1.supabase
        .from("qr_codes")
        .select("*")
        .eq("id", id)
        .single();
    if (error)
        throw error;
    const { count: signups } = await supabase_1.supabase
        .from("scores")
...
async function recordScan(ref) {
    ...
    await supabase_1.supabase.rpc("increment_qr_scan", { p_ref: ref });
    const base = env_1.env.FRONTEND_URL.replace(/\/$/, "");
